...
// // void parsePaths(IntegerVector cols_pos, IntegerVector counts_pos,
// //                  int nCases, int nControls, std::string file_path){
//
// /**
//  *
...
//   //       int col = cols_pos[j];
//   //       if(col < nCases){
//   //         int which_double_index = col/64;
//   //         path[which_double_index] |= one_64bit << col % 64;
//   //       }else{
//   //         int which_double_index = vlen + (col-nCases)/64;
//   //         path[which_double_index] |= one_64bit << (col-nCases) % 64;
//   //       }
//   //
//   //   }
//   //
...
//  */
// // [[Rcpp::export]]
// void JoinIndicesMethod1(IntegerVector counts_neg1, IntegerVector cols_neg1,
//                         IntegerVector counts_pos1, IntegerVector cols_pos1,
//                         IntegerVector counts_conflict1, IntegerVector cols_conflict1,
//                         IntegerVector counts_neg2, IntegerVector cols_neg2, IntegerVector locations_neg2,
//                         IntegerVector counts_pos2, IntegerVector cols_pos2, IntegerVector locations_pos2,
...
//                         NumericMatrix ValueTable, NumericMatrix queues4init, std::string path, int nCases, int nControls, int K,
...
//   std::vector<std::vector<uint64_t> > paths_pos1 = parsePaths(cols_pos1, counts_pos1, nCases, nControls, pos_path1);
//   std::string pos_path1 = path + "paths_pos1_" + IntToString(pathLength);
//   std::vector<std::vector<uint64_t> > paths_pos2 = parsePaths(cols_pos2, counts_pos2, nCases, nControls, pos_path2);
  ...
  //
  //   #pragma omp parallel for schedule(dynamic,1) shared(path_pos1,paths_pos2,flipped_pivot_length,location,count,ValueTable2,LocalIndicesQ,LocalQueues,CaseORControl2,new_cols_pos,new_counts_pos,new_locations_pos,ncols_pos,path_number,vlen,vlen2,nCases,nControls,iterations) if(pathLength > 3)
  //   for(int j = location; j < (location + count); j++){
  //
  ...
  //     }
  //
  //     VectorPriorityQueues::iterator it = tid_Queues.begin();
  //     for(int m = 0; m < iterations; m++, it++){
  //
  //       std::vector<uint64_t> &caseorcontrol = CaseORControl2[m];
  //       int cases_m = 0;
  //       int controls_m = 0;
...
List JoinIndicesMethod1(IntegerVector counts_neg1, IntegerVector cols_neg1,
  ...
  for(int i = 0; i < trguids2.size(); i++){
    ...
    for(int j = location; j < (location + count); j++){
      ...
      // std::vector<double> scores(iterations, 0);
      // std::vector<double> flipped_scores(iterations, 0);

      // VectorPriorityQueues::iterator it = tid_Queues.begin();
      // std::vector<double>::iterator it = scores.begin();
      // std::vector<double>::iterator it2 = flipped_scores.begin();
      for(int m = 0; m < iterations; m++){
        ...
        int new_cases_m = cases_m + (controls - controls_m);
        int new_controls_m = controls_m + (cases - cases_m);
        // scores[m] = ValueTable2[new_cases_m][new_controls_m];
        // flipped_scores[m] = ValueTable2[new_controls_m][new_cases_m];
        // (*it) = ValueTable2[new_cases_m][new_controls_m];
        // (*it2) = ValueTable2[new_controls_m][new_cases_m];
