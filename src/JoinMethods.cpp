...
List ProcessPaths(IntegerVector srcuids1, IntegerVector trguids1, List uids_CountLoc1, IntegerVector joining_gene_sign1,
  ...
  // Process Paths of length 2
    ...
    if(pathLength >= 2){
      ...
      Timer timer(method, vlen + vlen2, 2, iterations, nthreads);
  ...
  // Process Paths of length 3
    ...
    if(pathLength >= 3){
      ...
      Timer timer(method, vlen + vlen2, 3, iterations, nthreads);
...
timer.total_paths = total_paths;
  ...
  // Process Paths of length 4
    ...
    if(pathLength >= 4){
      ...
      Timer timer(method, vlen + vlen2, 4, iterations, nthreads);
      timer.total_paths = getTotalPaths(trguids4, uids_CountLoc4);
  ...
  // Process Paths of length 5
    ...
    if(pathLength >= 5){
      ...
      Timer timer(method, vlen + vlen2, 5, iterations, nthreads);
      timer.total_paths = getTotalPaths(trguids5, uids_CountLoc5);
